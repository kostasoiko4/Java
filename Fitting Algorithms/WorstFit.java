...
{ 
	...
	static ArrayList<Integer> worstFit(int sizeOfBlocks[], int sizeOfProcesses[])
	{
		...
		//Pick each process and the best block for it
		for(int i = 0; i < sizeOfProcesses.length; i++){
			//Best fit Block for This Process according to the algorythm
			int worstIndex = -255;
			for(int j = 0; j < sizeOfBlocks.length; j++){
				if(sizeOfBlocks[j] >= sizeOfProcesses[i]){
					if(worstIndex == -255){worstIndex = j;}
					else if(sizeOfBlocks[worstIndex] < sizeOfBlocks[j]){worstIndex = j;}
				}
			}
			if(worstIndex != -255){
				// Allocate block j to p[i] process
				memAlloc.remove(i);
				memAlloc.add(i,worstIndex);

				// Reduce available memory
				sizeOfBlocks[worstIndex] -= sizeOfProcesses[i];
			}
		}
		return memAlloc;
